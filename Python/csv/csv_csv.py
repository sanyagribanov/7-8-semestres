...
# Добавление данных
def insert(fio, age, curs, bil):
    global csv_file
    try:
        mx = max(csv_file, key=lambda x: int(x['ном']))
        csv_file.append({'ном': int(mx['ном']) + 1, 'фио': fio, 'возраст': age, 'курс': curs, 'студ.билет': bil})
...
def save():
    with open('data.csv', "w", encoding="utf-8", newline="") as file:
        columns = ['ном', 'фио', 'возраст', 'курс', 'студ.билет']
    ...
    try:
        if len(csv_file) == 0:
            ...
            print('{:<5}{:<25}{:<8}{:<12}{:<15}'.format(
                'ном', 'фио', 'возраст', 'курс', 'студ.билет'
            ))
            for el in csv_file:
